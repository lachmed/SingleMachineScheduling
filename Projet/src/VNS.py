...
# shaking : générer aléatoirement une séquance dans la structure de voisinnage numéro l
            ...
            x2=tmp 
            if calculerNbRetard(x2, data) < retardx1: 
...
#lieu de la séquance courante                
        ...
        if time.time() - start > 3: # si on dépasse un temps donné dans cette boucle alors arrêter (c'est la condition d'arrêt)            
            break
    print("Out of VND")
    return x1
...
#appliquer la recherche locale sur x1 
            ...
            if calculerNbRetard(x1, data)< retardx:
...
data=getData("../instances/P1_n10.txt")
